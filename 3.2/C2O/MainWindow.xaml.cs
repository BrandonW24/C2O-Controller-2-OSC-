...
namespace C2O
{
    public partial class MainWindow : Window
    {
        ...
        private VJoyMotionBridge? _vJoyMotionBridge;
        ...
        #region App LifeCycly and Icons
        ...
        #region Profile Management and Serialization
        ...
        #region Hardware Polling and OSC
                ...
                if (_isRunning && _motionSender != null)
                {
                    ...
                    if (motionEnabled && safetyEnabled && !_hasZeroedMotion)
                    {
                        if ((DateTime.UtcNow - _lastMotionReceiveTime).TotalMilliseconds > timeout)
                        {
                            ...
                            if(protocol == "vJoy" && _vJoyMotionBridge != null)
                            {
                                _vJoyMotionBridge.SendAxes(0, 0, 0, 0, 0);
                            }
        ...
        #region OSC Listener Loop (FFB & Motion Forwarding)
        private async Task OscListenerLoop(CancellationToken token)
        {
            ...
            try
            {
                ...
                while (!token.IsCancellationRequested)
                {
                    try 
                    {
                        ...
                        if (addr.Contains("/motion") && isMotionEnabled && _motionSender != null)
                        {
                            ...
                            if (args.Length > 0 && args[0] is float rawVal)
                            {
                                ...
                                switch(protocol)
                                {
                                    case CONSTANTS.MOTION_OUTPUTS.SIM_STRING:
                                        {
                                        ...
                                        break;
                                    case CONSTANTS.MOTION_OUTPUTS.OSC:
                                        {
                                            // Raw OSC Forward
                                        ...
                                        break;
                                    case CONSTANTS.MOTION_OUTPUTS.V_JOY:
                                        {
                                            switch(axisName)
                                            {
                                                case CONSTANTS.MOTION_AXES.ROLL:
                                                    vJoyValues[0] = finalOutputVal;
                                                    break;
                                                case CONSTANTS.MOTION_AXES.PITCH:
                                                    vJoyValues[1] = finalOutputVal;
                                                    break;
                                                case CONSTANTS.MOTION_AXES.HEAVE:
                                                    vJoyValues[2] = finalOutputVal;
                                                    break;
                                                case CONSTANTS.MOTION_AXES.SWAY:
                                                    vJoyValues[3] = finalOutputVal;
                                                    break;
                                                case CONSTANTS.MOTION_AXES.SURGE:
                                                    vJoyValues[4] = finalOutputVal;
                                                    break;
                                            }

                                            _vJoyMotionBridge?.SendAxes(
                                                vJoyValues[0],
                                                vJoyValues[1],
                                                vJoyValues[2],
                                                vJoyValues[3],
                    ...
                    {
                        LogScroll($"[OSC IN ERROR] {ex.ToString()}");
                    }
                }
            }
            ...
            {
                LogScroll($"[LISTENER ERROR] {e.ToString()}");
            }
        }

        #endregion

        #region Hardware Effect Routing Methods
        ...
        #region UI Updates & Stream Controls 
        ...
        private void StartStreaming()
        {
            ...
            if (ChkMotionEnable.IsChecked == true)
            {
                if (int.TryParse(MotionPortEntry.Text, out int mPort))
                {
                    _motionSender = new UdpClient(MotionIpEntry.Text, mPort);
                }

                if(VJoyMotionBridge.VJoyEnabled())
                {
                    _vJoyMotionBridge = new VJoyMotionBridge();
                    if(!_vJoyMotionBridge.Begin())
                    {
                        _vJoyMotionBridge.End();
                        _vJoyMotionBridge = null;
                        LogScroll("Failed to connect to vJoy");
                    }
                }
                else
                {
                    LogScroll("vJoy Installation Not Found");
                }
            }
